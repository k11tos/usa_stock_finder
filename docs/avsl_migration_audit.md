...
> `check_avsl_sell_signal()`, and the retired post-run comparison
...
- `UsaStockFinder.check_avsl_sell_signal()` uses the live AVSL path and compares the latest close against the latest positive finite `original_avsl` value using strict `current_price < original_avsl` semantics.
...
signal. At that time, an older approximation path was
...
- **Original AVSL could be closer to current price than older AVSL.** Those cases were reviewed as possible earlier-sell triggers during migration.
...
- The retired post-run comparison monitor and `tools.compare_avsl` path were removed after the live migration.
